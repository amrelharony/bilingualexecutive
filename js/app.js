...
// Capture install prompt as early as possible (before Alpine init)
...
window._toast = function(message, type = 'info') {
    const clean = String(message).replace(/\n/g, ' ').trim();
    Alpine.store('gamification')?._queueToast({ type, message: clean });
};

document.addEventListener('alpine:init', () => {

    // Global Store: shared state accessible by all components via $store.app
    Alpine.store('app', {
        supabaseUrl: 'https://qbgfduhsgrdfonxpqywu.supabase.co',
        supabaseKey: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFiZ2ZkdWhzZ3JkZm9ueHBxeXd1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjczNjQ0MzcsImV4cCI6MjA4Mjk0MDQzN30.0FGzq_Vg2oYwl8JZXBrAqNmqTBWUnzJTEAdgPap7up4',
        supabase: null,
        cfBase: 'https://pub-fafafe2a62594937b094305a3b9ef698.r2.dev',
        ghBase: 'https://amrelharony.github.io/bilingualexecutive/assets',
        isVipMode: false,
        vipTerminalActive: false,
        currentTab: 'dashboard',
        currentGroup: 'radar',
        viewMode: 'academy',
        isMobile: false,

        // Theme: 'auto' follows time-of-day, 'dark'/'light' are manual overrides
        themeMode: localStorage.getItem('bilingual_theme') || 'auto',
        isDaylight: false,

        get effectiveTheme() {
            if (this.themeMode === 'auto') return this.isDaylight ? 'light' : 'dark';
            return this.themeMode;
        },

        initTheme() {
            this._updateDaylightFlag();
            this._applyTheme();
            // Re-check every minute for auto-transitions
            setInterval(() => {
                this._updateDaylightFlag();
        ...
        _updateDaylightFlag() {
            ...
            this.isDaylight = hour >= 6 && hour < 18;
        },

        toggleTheme() {
            const cycle = { auto: 'light', light: 'dark', dark: 'auto' };
            this.themeMode = cycle[this.themeMode];
            localStorage.setItem('bilingual_theme', this.themeMode);
            this._applyTheme();
        },
        ...
        _applyTheme() {
            ...
            document.querySelector('meta[name="theme-color"]')
                ?.setAttribute('content', theme === 'light' ? '#cdd9ea' : '#0f172a');
            window.dispatchEvent(new CustomEvent('theme-changed', { detail: theme }));
        },

        get themeIcon() {
            if (this.themeMode === 'auto') return this.isDaylight ? 'fa-sun' : 'fa-moon';
            return this.themeMode === 'light' ? 'fa-sun' : 'fa-moon';
        },

        get themeLabel() {
            const labels = { auto: 'Auto', light: 'Day', dark: 'Night' };
            return labels[this.themeMode];
        },

        copyToClipboard(text, label) {
            navigator.clipboard.writeText(text).then(() => {
                Alpine.store('haptics')?.fire('confirm');
                const msg = label ? `${label} copied!` : 'Copied!';
                const el = document.createElement('div');
                el.className = 'fixed bottom-20 left-1/2 -translate-x-1/2 bg-white/10 backdrop-blur-xl border border-white/20 text-white text-xs font-mono px-4 py-2 rounded-full z-[999] shadow-glass';
                el.textContent = msg;
    ...
    author: "Amr Elharony",
    authorWebsite: "https://amrelharony.com",
    amazonLink: "https://www.amazon.com/dp/B0GKWZKFXC",
    spotifyLink: "https://open.spotify.com/show/1Y2nfqQJ0gN2bdxRTy8KRX",
    coverImage: "assets/book-cover.png",
    modelSrc: "assets/book.glb",
    modelIos: "assets/book.usdz",
...
renderPage(num) {
    ...
    rawPdfDoc.getPage(num).then((page) => {
        ...
        }).catch(() => { this.pdfState.pageRendering = false; });
    }).catch(() => { this.pdfState.pageRendering = false; });
    this.pdfState.pageNum = num;
},

_teardownPdf() {
    if (this.pdfViewer.observer) {
        this.pdfViewer.observer.disconnect();
        this.pdfViewer.observer = null;
    }
    const inner = document.querySelector('.pdf-scroll-inner');
    if (inner) inner.innerHTML = '';
    this.pdfViewer.renderedPages = {};
    this._removePinchZoom();
    if (rawPdfDoc) { try { rawPdfDoc.destroy(); } catch(e) {} rawPdfDoc = null; }
    if (this.pdfViewer._scrollNavTimeout) clearTimeout(this.pdfViewer._scrollNavTimeout);
},
...
closeViewer() {
    ...
    this._teardownPdf();
    this._resetImageViewer();
    this._removeImageViewerListeners();
...
_initContentProtection() {
    ...
    const showGuard = () => guard.classList.add('active');
    const hideGuard = () => guard.classList.remove('active');

    // Layer 1: Visibility change (app switch, screen lock, control center)
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'hidden') showGuard();
        else setTimeout(hideGuard, 300);
    });

    // Layer 2: Window blur/focus (task switcher, notification shade pull)
    window.addEventListener('blur', showGuard);
    window.addEventListener('focus', () => setTimeout(hideGuard, 300));

    // Layer 3: iOS screenshot multi-touch detection
    // When the user presses Power + Volume, iOS briefly fires
    // a touchcancel or the touch count changes unexpectedly.
    if (this.isMobile) {
        let activeTouches = 0;
        document.addEventListener('touchstart', (e) => {
            activeTouches = e.touches.length;
            if (activeTouches >= 3) showGuard();
        }, { passive: true });
        document.addEventListener('touchend', () => { activeTouches = 0; }, { passive: true });
        document.addEventListener('touchcancel', () => {
            showGuard();
            setTimeout(hideGuard, 1500);
        }, { passive: true });
    }

    // Layer 4: Screen Capture / Screen Recording detection
    if (navigator.mediaDevices) {
        const checkCapture = () => {
            try {
                navigator.mediaDevices.enumerateDevices().then(devices => {
                    const isRecording = devices.some(d =>
                        d.kind === 'videoinput' && d.label.toLowerCase().includes('screen')
                    );
                    if (isRecording) showGuard();
                });
            } catch (_) {}
        };
        navigator.mediaDevices.addEventListener?.('devicechange', checkCapture);
    }

    // Layer 5: Prevent keyboard print-screen (desktop fallback)
    ...
    // Layer 6: Prevent drag-save on all images
...
_showBookTransition(onComplete) {
    const el = document.getElementById('app-preloader');
    ...
    el.offsetHeight;
    APP_ANIM.animateBookLoader(onComplete);
},
...
_clampImageZoom(z) {
    return Math.min(this.imageViewer.maxZoom, Math.max(this.imageViewer.minZoom, z));
},

_applyImageTransform() {
    const el = document.getElementById('image-viewer-img');
    if (!el) return;
    const iv = this.imageViewer;
    el.style.transform = `translate(${iv.panX}px, ${iv.panY}px) scale(${iv.zoom})`;
},
...
_clampPan() {
    ...
    const displayedW = img.offsetWidth * iv.zoom;
    const displayedH = img.offsetHeight * iv.zoom;

    if (displayedW <= cRect.width) {
        iv.panX = 0;
    } else {
        const maxPanX = (displayedW - cRect.width) / 2;
        iv.panX = Math.min(maxPanX, Math.max(-maxPanX, iv.panX));
    }
    if (displayedH <= cRect.height) {
        iv.panY = 0;
    } else {
        const maxPanY = (displayedH - cRect.height) / 2;
        iv.panY = Math.min(maxPanY, Math.max(-maxPanY, iv.panY));
    }
},

imgZoomIn() {
    const iv = this.imageViewer;
    iv.zoom = this._clampImageZoom(iv.zoom * 1.3);
    this._clampPan();
...
imgToggleZoom() {
    Alpine.store('haptics')?.fire('tap');
    if (this.imageViewer.zoom > 1.05) {
        this.imgZoomReset();
    } else {
        const iv = this.imageViewer;
        iv.zoom = 2.5;
...
imgZoomToPoint(delta, clientX, clientY) {
    const iv = this.imageViewer;
    const container = document.getElementById('image-viewer-container');
    if (!container) return;
    ...
    iv.zoom = this._clampImageZoom(oldZoom * factor);
    const scale = iv.zoom / oldZoom;
    iv.panX = cx - scale * (cx - iv.panX);
    iv.panY = cy - scale * (cy - iv.panY);
    this._clampPan();
    this._applyImageTransform();
},
...
_imgPointerDown(e) {
    ...
    const iv = this.imageViewer;
    iv.dragging = true;
    iv.dragStartX = e.clientX;
    iv.dragStartY = e.clientY;
    iv.panStartX = iv.panX;
    iv.panStartY = iv.panY;
},

_imgPointerMove(e) {
    const iv = this.imageViewer;
    if (!iv.dragging) return;
    iv.panX = iv.panStartX + (e.clientX - iv.dragStartX);
    iv.panY = iv.panStartY + (e.clientY - iv.dragStartY);
    this._clampPan();
    this._applyImageTransform();
},

_imgPointerUp() {
    this.imageViewer.dragging = false;
},
...
_imgTouchStart(e) {
    ...
    if (e.touches.length === 2) {
        ...
        iv.pinchStartDist = Math.hypot(dx, dy);
        iv.pinchStartZoom = iv.zoom;
    } else if (e.touches.length === 1) {
        const now = Date.now();
        if (now - iv.lastTapTime < 300) {
            e.preventDefault();
            iv.lastTapTime = 0;
            this.imgToggleZoom();
            return;
        ...
        iv.lastTapTime = now;
        iv.dragging = true;
        iv.dragStartX = e.touches[0].clientX;
        iv.dragStartY = e.touches[0].clientY;
        iv.panStartX = iv.panX;
        iv.panStartY = iv.panY;
    }
},
...
_imgTouchMove(e) {
    ...
    if (e.touches.length === 2) {
        ...
        const scale = dist / iv.pinchStartDist;
        iv.zoom = this._clampImageZoom(iv.pinchStartZoom * scale);
        this._clampPan();
        this._applyImageTransform();
    } else if (e.touches.length === 1 && iv.dragging && iv.zoom > 1.05) {
        e.preventDefault();
        iv.panX = iv.panStartX + (e.touches[0].clientX - iv.dragStartX);
        iv.panY = iv.panStartY + (e.touches[0].clientY - iv.dragStartY);
        this._clampPan();
        this._applyImageTransform();
    }
},
...
_imgWheel(e) {
    e.preventDefault();
    this.imgZoomToPoint(e.deltaY, e.clientX, e.clientY);
},

// Keyboard shortcuts
_imgKeydown(e) {
    if (this.viewer.type !== 'image' || !this.viewer.active) return;
    switch (e.key) {
        case '+': case '=': e.preventDefault(); this.imgZoomIn(); break;
        case '-': case '_': e.preventDefault(); this.imgZoomOut(); break;
        case '0': e.preventDefault(); this.imgZoomReset(); break;
        case 'Escape': e.preventDefault(); this.closeViewer(); break;
    }
},

_setupImageViewerListeners() {
    this._boundImgKeydown = (e) => this._imgKeydown(e);
    document.addEventListener('keydown', this._boundImgKeydown);
},

_removeImageViewerListeners() {
    if (this._boundImgKeydown) {
        document.removeEventListener('keydown', this._boundImgKeydown);
        this._boundImgKeydown = null;
    }
},
...
async downloadImage() {
    ...
    try {
        const resp = await fetch(this.viewer.url);
        const blob = await resp.blob();
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        const ext = this.viewer.url.split('.').pop()?.split('?')[0] || 'png';
        a.download = `${this.viewer.title || 'image'}.${ext}`;
...
initPdfScrollViewer() {
    ...
    if (!container || !rawPdfDoc) return;
    let inner = container.querySelector('.pdf-scroll-inner');
    if (!inner) {
        inner = document.createElement('div');
        inner.className = 'pdf-scroll-inner py-4 px-2 md:px-4';
        container.appendChild(inner);
    }
    inner.innerHTML = '';
    this.pdfViewer.renderedPages = {};
    this.pdfViewer.zoom = 1;

    const containerWidth = container.clientWidth || window.innerWidth;
    const baseWidth = Math.min(containerWidth - 24, 900);
    this.pdfViewer._basePageWidth = baseWidth;
    const placeholderH = Math.round(baseWidth * 0.75);

    for (let i = 1; i <= rawPdfDoc.numPages; i++) {
        const wrapper = document.createElement('div');
        wrapper.className = 'pdf-page-wrapper';
        wrapper.dataset.page = i;
        wrapper.dataset.aspect = '0.75';
        wrapper.style.cssText = `position:relative;margin:0 auto 12px;background:#1e293b;border-radius:8px;overflow:hidden;border:1px solid rgba(255,255,255,0.06);width:${baseWidth}px;height:${placeholderH}px;`;
        const canvas = document.createElement('canvas');
        canvas.className = 'pdf-page-canvas';
        canvas.dataset.page = i;
        canvas.style.cssText = `display:block;width:${baseWidth}px;height:${placeholderH}px;`;
        const label = document.createElement('div');
        label.style.cssText = 'position:absolute;bottom:8px;right:10px;font:9px/1 ui-monospace,monospace;color:rgba(148,163,184,0.6);pointer-events:none;';
        label.textContent = `${i} / ${rawPdfDoc.numPages}`;
        wrapper.appendChild(canvas);
        wrapper.appendChild(label);
        inner.appendChild(wrapper);
    }

    if (this.pdfViewer.observer) this.pdfViewer.observer.disconnect();
    this.pdfViewer.observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                const pageNum = parseInt(entry.target.dataset.page, 10);
                if (!this.pdfViewer.renderedPages[pageNum]) {
                    this._renderScrollPage(pageNum);
                }
            }
        });
        if (!this.pdfViewer._scrollNavTimeout) this._updateScrollPageNum();
    }, { root: container, rootMargin: '300px 0px', threshold: 0.01 });

    inner.querySelectorAll('.pdf-page-wrapper').forEach(w => {
        this.pdfViewer.observer.observe(w);
    });

    this._setupPinchZoom(container);
},

_renderScrollPage(num) {
    if (!rawPdfDoc || this.pdfViewer.renderedPages[num]) return;
    this.pdfViewer.renderedPages[num] = 'pending';
    const zoomAtCall = this.pdfViewer.zoom;
    rawPdfDoc.getPage(num).then((page) => {
        if (this.pdfViewer.zoom !== zoomAtCall) {
            this.pdfViewer.renderedPages[num] = false;
            return;
        }
        const canvas = document.querySelector(`canvas.pdf-page-canvas[data-page="${num}"]`);
        if (!canvas) return;
        const baseW = this.pdfViewer._basePageWidth || 800;
        const displayW = Math.round(baseW * zoomAtCall);
        const viewport = page.getViewport({ scale: 1 });
        const aspectRatio = viewport.height / viewport.width;
        const displayH = Math.round(displayW * aspectRatio);
        const dpr = Math.min(window.devicePixelRatio || 1, 2);
        const renderScale = (displayW / viewport.width) * dpr;
        ...
        wrapper.dataset.aspect = String(aspectRatio);
        wrapper.style.width = displayW + 'px';
        wrapper.style.height = displayH + 'px';
        page.render({ canvasContext: canvas.getContext('2d'), viewport: renderViewport }).promise.then(() => {
            this.pdfViewer.renderedPages[num] = true;
        }).catch(() => { this.pdfViewer.renderedPages[num] = false; });
    }).catch(() => { this.pdfViewer.renderedPages[num] = false; });
},

_updateScrollPageNum() {
    const container = document.getElementById('pdf-scroll-container');
    if (!container) return;
    ...
    wrappers.forEach(w => {
        ...
        if (rect.top <= targetY) currentPage = parseInt(w.dataset.page, 10);
    });
    this.pdfState.pageNum = currentPage;
},

_setupPinchZoom(container) {
    this._removePinchZoom();
    let startDist = 0;
    let startZoom = 1;
    ...
    const onTouchMove = (e) => {
        if (e.touches.length === 2 && startDist > 0) {
            ...
            const newZoom = Math.min(3, Math.max(0.5, startZoom * (dist / startDist)));
            this.pdfViewer.zoom = Math.round(newZoom * 100) / 100;
            this._applyZoom();
        }
    };
    ...
    this.pdfViewer._pinchHandlers = { el: container, onTouchStart, onTouchMove, onTouchEnd };
},

_removePinchZoom() {
    const h = this.pdfViewer._pinchHandlers;
    if (h) {
        ...
        this.pdfViewer._pinchHandlers = null;
    }
},

scrollToPage(num) {
    if (this.pdfViewer._scrollNavTimeout) clearTimeout(this.pdfViewer._scrollNavTimeout);
    this.pdfViewer._scrollNavTimeout = setTimeout(() => { this.pdfViewer._scrollNavTimeout = null; }, 600);
    const container = document.getElementById('pdf-scroll-container');
    const target = container?.querySelector(`.pdf-page-wrapper[data-page="${num}"]`);
    if (target) target.scrollIntoView({ behavior: 'smooth', block: 'start' });
    this.pdfState.pageNum = num;
},

pdfZoomIn() {
    this.pdfViewer.zoom = Math.min(3, this.pdfViewer.zoom + 0.25);
    this._applyZoom();
},

pdfZoomOut() {
    this.pdfViewer.zoom = Math.max(0.5, this.pdfViewer.zoom - 0.25);
    this._applyZoom();
},
...
_applyZoom() {
    ...
    const wrappers = container.querySelectorAll('.pdf-page-wrapper');
    const baseW = this.pdfViewer._basePageWidth || 800;
    const displayW = Math.round(baseW * this.pdfViewer.zoom);
    wrappers.forEach(w => {
        const canvas = w.querySelector('canvas');
        if (!canvas) return;
        const aspectRatio = parseFloat(w.dataset.aspect) || 0.75;
        const displayH = Math.round(displayW * aspectRatio);
        canvas.style.width = displayW + 'px';
        canvas.style.height = displayH + 'px';
    ...
    prevPage() {
        ...
        Alpine.store('haptics')?.fire('pageFlip');
        this.pdfState.pageNum--;
        if (this.pdfViewer.mode === 'scroll') this.scrollToPage(this.pdfState.pageNum);
        else this.queueRenderPage(this.pdfState.pageNum);
    },
        ...
        async downloadChapterAssets(chapter) {
            ...
            if (!quota.chapters.includes(chapter.id)) {
                ...
                if (quota.chapters.length >= dm.limit) {
                    _toast(`Daily Download Limit Reached (${dm.limit}/day). Come back tomorrow.`, 'warning');
                    return;
                }
            ...
            // 4. Trigger Downloads
            _toast(`Downloading Asset Pack for Chapter ${chapter.id}...`, 'info');
       ...
       // --- CACHING ACTIONS (FAULT TOLERANT) ---
        async toggleOfflineCache(chapter) {
            ...
            if (om.isCached(chapter.id)) {
                _toast("This chapter is already saved.", 'info');
                return;
            }

            const quota = om.getQuota();
            if (quota.downloadedCount >= om.limit) {
                _toast(`Daily Limit Reached (${om.limit}/day).`, 'warning');
                return;
            }
            ...
            try {
                ...
                if (successCount > 0) {
                    ...
                    _toast(`Saved ${successCount} assets for offline use!`, 'success');
                } else {
                ...
                _toast("Download failed. Check internet or file availability.", 'error');
            } finally {
                om.loading = null;
            }
        },
        ...
        // ── Global Viewer API ─────────────────────────────────
        async openViewer(url, title, type) {
            if (!url) return;
            this._teardownPdf();
            const isPdf = type === 'pdf' || (!type && /\.pdf(\?|$)/i.test(url));
            this.viewer.title = title || '';
            this.viewer.type = isPdf ? 'pdf' : 'image';
            ...
            if (isPdf) {
                ...
                this.pdfViewer.zoom = 1;
                try {
                    rawPdfDoc = await pdfjsLib.getDocument(url).promise;
                    this.pdfState.numPages = rawPdfDoc.numPages;
                    this.viewer.loading = false;
                    await this.$nextTick();
                    if (this.pdfViewer.mode === 'scroll') {
                        setTimeout(() => this.initPdfScrollViewer(), 50);
                    } else {
                ...
                await this.$nextTick();
                this.viewer.url = url;
                this._setupImageViewerListeners();
            }
        },
        ...
        async viewPdf(chapter) {
            ...
            await this.openViewer(finalUrl, `${chapter.title} - Slides`, 'pdf');
            if (this.viewer.active) Alpine.store('gamification').trackPdfView();
        },

        async readBookPdf() {
            const url = this.bookConfig?.pdfPath;
            await this.openViewer(url, 'Executive Summary', 'pdf');
        },

        async viewImage(chapter, type) {
            let finalUrl = this.getImageUrl(chapter, type);
            if (this.offlineManager.isCached(chapter.id) || !navigator.onLine) {
                try {
                    const cacheName = this.offlineManager.cacheName || 'bilingual-content-v1';
                    const cache = await caches.open(cacheName);
            ...
            await this.openViewer(finalUrl, `${chapter.title} - ${type.toUpperCase()}`, 'image');
        },

        // Navigation
        async openChapter(id) {
    Alpine.store('haptics')?.fire('heavy');
    ...
    if (this.acPlayer && this.acPlayer.destroy) {
        this.acPlayer.destroy();
        this.acPlayer = null;
    }
    if (this.acUpdateInterval) {
        clearInterval(this.acUpdateInterval);
        this.acUpdateInterval = null;
    }

    this.acPlaying = false;
    this.acCurrentTime = 0;
    this.acDuration = 0;

    const gen = ++this._acGeneration;

    await this._ensureYouTubeAPI();

    this.$nextTick(() => {
        if (gen !== this._acGeneration) return;
        ...
        this.acPlayer = new YT.Player('academy-player', {
            ...
            events: {
                onReady: (event) => {
                    this.acDuration = event.target.getDuration();
                    this.acVolume = 50;
                ...
                onStateChange: (event) => {
                    if (event.data === YT.PlayerState.PLAYING) {
                        this.acPlaying = true;
                        if (this.acUpdateInterval) clearInterval(this.acUpdateInterval);
                        this.acUpdateInterval = setInterval(() => {
                            if (this.acPlayer && this.acPlayer.getCurrentTime) {
                                this.acCurrentTime = this.acPlayer.getCurrentTime();
                            }
                        }, 250);
                    } else {
                        this.acPlaying = false;
        ...
        closeChapter() {
    Alpine.store('haptics')?.fire('dismiss');
...
// --- ACADEMY PROGRESS ENGINE ---
        ...
        completeActiveChapter() {
            ...
            Alpine.store('haptics')?.fire('success');
            ...
            Alpine.store('gamification').trackChapterComplete(currentId);
            if (Alpine.store('academy')) Alpine.store('academy').logActivity('chapter_complete');
            Alpine.store('gamification')._queueToast({ type: 'info', message: nextMessage });
            this.closeChapter();
        },
        
       // Flashcard Engine
        async launchFlashcards(chapter) {
            if (Alpine.store('academy')) Alpine.store('academy').logActivity('flashcard_launch');
        ...
        nextCard() {
            ...
            this.isFlipped = false;
            Alpine.store('gamification').trackFlashcard();
        ...
        init() {
            ...
            let storedVip = false;
            try { storedVip = localStorage.getItem('vip_granted') === '1'; } catch(e) {}

            if (storedVip) {
                this.isVipMode = true;
                this.showVipWelcomeBack();
            } else if (urlCode === 'vip_nfc_001') {
                this.openVipTerminal(urlCode);
            }
...
if (hasEnteredBefore) {
    ...
    this.$nextTick(() => {
        APP_ANIM.animateBookLoader(() => {
            if (this.viewMode === 'academy') APP_ANIM.animateMetroMap();
            else if (this.viewMode === 'tools') APP_ANIM.animateToolCards();
            else if (this.viewMode === 'ebook') APP_ANIM.animateBookHero();
        });
    });
} else {
    this.$nextTick(() => {
        APP_ANIM.animateLanding();
        this._hidePreloaderForLanding();
    });
}
...
if (window.supabase) {
    ...
    Alpine.store('app').supabase = this.supabase;
    Alpine.store('gamification').init();
    Alpine.store('gamification').pullFromSupabase();
    Alpine.store('presence').init();
    Alpine.store('guestbook').init();
    Alpine.store('visitorStats').init();
} else {
...
if (Alpine.store('academy')) {
    Alpine.store('academy')._metroMapRef = this.metroMap;
}
            ...
            // Pick up any prompt captured before Alpine init
            if (_deferredInstallPrompt) this.deferredPrompt = _deferredInstallPrompt;
            window.addEventListener('beforeinstallprompt', (e) => {
                e.preventDefault();
    ...
    this.$nextTick(() => APP_ANIM.initScrollReveal());

    this.$watch('navVisible', (val) => {
        const nav = document.querySelector('.fixed.bottom-0.left-0.right-0.z-\\[40\\]');
        if (nav) APP_ANIM.animateNavBar(nav, val);
        APP_ANIM.animateAutoHide(val);
    });
            ...
            this.$watch('viewMode', (val, oldVal) => {
                if (val !== 'academy' && Alpine.store('academy')) {
                    Alpine.store('academy').closeQuiz();
                    Alpine.store('academy').closeNotes();
                    Alpine.store('academy').closeAiTutor();
                }
                this.$nextTick(() => {
                    if (val === 'ebook') APP_ANIM.animateBookHero();
                    else if (val === 'academy') APP_ANIM.animateMetroMap();
                    else if (val === 'tools') APP_ANIM.animateToolCards();
                    APP_ANIM.refreshScrollTrigger();
                });
            });

            this.$watch('currentGroup', () => {
                this.$nextTick(() => {
                    if (this.viewMode === 'tools' && this.currentTab === 'dashboard') APP_ANIM.animateToolCards();
                });
            });

            this.$watch('activeChapterId', (val, oldVal) => {
                if (oldVal && !val && this.viewMode === 'academy') {
                    this.$nextTick(() => APP_ANIM.animateMetroMap());
                }
            });

            // Talent chart + Presence tracking watchers
this.$watch('currentTab', (value) => {
    if (value === 'talent') {
        setTimeout(() => { this.updateTalentChart(); }, 200);
    }
    if (value === 'dashboard' && this.viewMode === 'tools') {
        this.$nextTick(() => APP_ANIM.animateToolCards());
    }
    Alpine.store('presence').trackToolSwitch(value, Alpine.store('app').currentGroup);
    Alpine.store('visitorStats').trackToolUse(value);
});
...
window.addEventListener('theme-changed', () => {
    if (this.currentTab === 'talent') this.updateTalentChart();
});
...
_ensureYouTubeAPI() {
    ...
    if (window.YT && window.YT.Player) {
        this._ytApiPromise = Promise.resolve();
...
_landingPlayerReady: false,
_landingPlayerLoading: false,
async _initLandingPlayer() {
    if (this.player || this._landingPlayerLoading) return;
    this._landingPlayerLoading = true;

    await this._ensureYouTubeAPI();
    if (!this.showLanding || !document.getElementById('youtube-player')) {
        this._landingPlayerLoading = false;
        return;
    }
    ...
    this.player = new YT.Player('youtube-player', {
        ...
        playerVars: {
            ...
            modestbranding: 1, loop: 0, playlist: '8GvrODrkQ7M',
            playsinline: 1, enablejsapi: 1
        },
        events: {
            onReady: (event) => {
                this._landingPlayerReady = true;
...
initYouTubePlayer() {
    this.initFullscreenListeners();
        ...
        acDuration: 0,
        acVolume: 50,
        acVolumeBeforeMute: 50,
        acUpdateInterval: null,
        acControlsVisible: false,
        acControlsTimer: null,
        _acGeneration: 0,
...
tools: {
    radar: [
        ...
        { id: 'stats', label: 'Visitor Stats', desc: 'Live platform analytics & visitor intelligence.', icon: 'fa-solid fa-chart-column', color: 'text-indigo-400' }
    ],
    academy: [
       ...
       dashboardTools: [ 
         ...
         { id: 'adaptation', label: 'Adaptation Health', desc: 'Measure organizational plasticity and rigidity.', icon: 'fa-solid fa-dna', color: 'text-cyan-400', vip: false },
           { id: 'dt_tracker', label: 'Transformation ROI', desc: 'Track the J-Curve: Hard costs vs. Soft value realization.', icon: 'fa-solid fa-chart-line', color: 'text-green-400', vip: false },
            ...
            { id: 'architect', label: 'Architect Console', desc: 'Access High-Level Scripts.', icon: 'fa-solid fa-terminal', color: 'text-[#00ff41]', vip: true },
...
seekVideo(sliderValue) {
    Alpine.store('haptics')?.fire('slide');
...
toggleVideoPlay() {
    Alpine.store('haptics')?.fire('doubleTap');
...
toggleViewerFullscreen() {
    Alpine.store('haptics')?.fire('heavy');
    const el = document.querySelector('.fixed.inset-0.z-\\[150\\]');
    if (!el) return;
...
initFullscreenListeners() {
    document.addEventListener('fullscreenchange', () => {
        ...
        this.viewerFullscreen = !!document.fullscreenElement;
    });
    
    document.addEventListener('webkitfullscreenchange', () => {
        this.isFullscreen = !!document.webkitFullscreenElement;
        this.viewerFullscreen = !!document.webkitFullscreenElement;
    });
    
    document.addEventListener('msfullscreenchange', () => {
        this.isFullscreen = !!document.msFullscreenElement;
        this.viewerFullscreen = !!document.msFullscreenElement;
    });
},
...
acTogglePlay() {
    Alpine.store('haptics')?.fire('doubleTap');
    if (this.acPlayer && this.acPlayer.getPlayerState) {
        if (this.acPlaying) {
            this.acPlayer.pauseVideo();
            this.acPlaying = false;
            if (this.acUpdateInterval) {
                clearInterval(this.acUpdateInterval);
                this.acUpdateInterval = null;
...
acSeek(sliderValue) {
    Alpine.store('haptics')?.fire('slide');
    const newTime = parseFloat(sliderValue);
    if (this.acPlayer && this.acPlayer.seekTo) {
        this.acPlayer.seekTo(newTime, true);
        this.acCurrentTime = newTime;
    }
},

acSetVolume(newVolume) {
    Alpine.store('haptics')?.fire('slide');
    const volume = parseInt(newVolume, 10);
    this.acVolume = volume;
    if (this.acPlayer && this.acPlayer.setVolume) {
        this.acPlayer.setVolume(volume);
        if (volume === 0) {
            this.acMuted = true;
        } else {
            this.acMuted = false;
            this.acVolumeBeforeMute = volume;
        }
    }
},
...
acToggleMute() {
    ...
    if (this.acPlayer) {
        if (this.acMuted) {
            if (this.acPlayer.unMute) this.acPlayer.unMute();
            if (this.acPlayer.setVolume) this.acPlayer.setVolume(this.acVolumeBeforeMute || 50);
            this.acMuted = false;
            this.acVolume = this.acVolumeBeforeMute || 50;
        } else {
            this.acVolumeBeforeMute = this.acVolume;
            if (this.acPlayer.mute) this.acPlayer.mute();
            this.acMuted = true;
            this.acVolume = 0;
        }
    }
...
acShowControls() {
    this.acControlsVisible = true;
    if (this.acControlsTimer) clearTimeout(this.acControlsTimer);
    this.acControlsTimer = setTimeout(() => {
        this.acControlsVisible = false;
    }, 3000);
},
  ...
  enterApp() {
    ...
    APP_ANIM.killLanding();

    this._showBookTransition(() => {
        if (this.viewMode === 'academy') APP_ANIM.animateMetroMap();
        else if (this.viewMode === 'tools') APP_ANIM.animateToolCards();
        else if (this.viewMode === 'ebook') APP_ANIM.animateBookHero();
    });
...
setupActivityTracking() {
    ...
    if (this._activityListeners) {
        this._activityListeners.forEach(([evt, fn]) => document.removeEventListener(evt, fn));
    }

    const handler = () => this.trackUserActivity();
    this._activityListeners = ['click', 'scroll', 'keydown'].map(evt => {
        document.addEventListener(evt, handler, { passive: true });
        return [evt, handler];
    });

    this.$watch('currentTab', handler);
    this.$watch('currentGroup', handler);
},
...
copyToClipboard(text, label = "Content") {
    if (!text || text.trim() === '') {
        _toast(`Nothing to copy for ${label}`, 'warning');
        return;
    }
    ...
    if (navigator.clipboard && window.isSecureContext) {
        navigator.clipboard.writeText(text)
            ...
            .catch((err) => {
                ...
                _toast(`Failed to copy ${label}. Please copy manually.`, 'error');
            });
    } else {
        ...
        openVipTerminal(prefill) {
            this.vipTerminal.active = true;
            Alpine.store('app').vipTerminalActive = true;
            this.vipTerminal.phase = 'prompt';
            this.vipTerminal.code = prefill || '';
            this.vipTerminal.result = '';
            this.vipTerminal.bootLines = [];
            this.vipTerminal._skipBoot = false;
            this.vipTerminal._submitting = false;
            this.$nextTick(() => {
                this.$refs.vipInput?.focus();
                if (prefill) setTimeout(() => this.submitVipCode(), 600);
            });
        },

        submitVipCode() {
            if (this.vipTerminal._submitting) return;
            if (this.vipTerminal.code.trim() === 'vip_nfc_001') {
                Alpine.store('haptics')?.fire('success');
                this.vipTerminal._submitting = true;
                this.vipTerminal.result = 'granted';
                this.$nextTick(() => {
                    const grantEl = document.querySelector('[x-show="vipTerminal.result === \'granted\'"]');
                    if (grantEl) APP_ANIM.animateVipGrant(grantEl);
                });
                setTimeout(() => this.runBootSequence(), 800);
            } else {
                Alpine.store('haptics')?.fire('error');
                this.vipTerminal.result = 'denied';
                setTimeout(() => { this.vipTerminal.result = ''; }, 2000);
            }
        },

        async runBootSequence() {
            this.vipTerminal.phase = 'boot';
            this.vipTerminal.bootLines = [];
            const lines = [
                '> ESTABLISHING SECURE TUNNEL...',
                '[OK]',
                '> DECRYPTING VAULT PARTITION...',
                '[OK]',
                '> LOADING SIMULATION ENGINES...',
                '[OK]',
                '> MOUNTING /dev/architect...',
                '[OK]',
            ...
            for (let i = 0; i < lines.length; i++) {
                if (this.vipTerminal._skipBoot) return;
                Alpine.store('haptics')?.fire('terminal');
                this.vipTerminal.bootLines.push(lines[i]);
                await new Promise(r => setTimeout(r, lines[i].startsWith('>') ? 500 : 300));
            }

            if (this.vipTerminal._skipBoot) return;

            this.vipTerminal.phase = 'ascii';
            this.$nextTick(() => {
                const asciiBox = document.querySelector('[x-show="vipTerminal.phase === \'ascii\'"]');
                if (asciiBox) APP_ANIM.animateVipAscii(asciiBox);
            });
        ...
        skipBoot() {
            if (this.vipTerminal._skipBoot) return;
            Alpine.store('haptics')?.fire('press');
            this.vipTerminal._skipBoot = true;
            this.spawnMatrixRain();
            this.vipTerminal.phase = 'rain';
            setTimeout(() => this.activateVip(), 800);
        },
        ...
        activateVip() {
            ...
            this.isVipMode = true;
            try {
                localStorage.setItem('vip_granted', '1');
                if (!localStorage.getItem('vip_session_start')) {
                    localStorage.setItem('vip_session_start', String(Date.now()));
                }
            } catch(e) {}
            this.vipTerminal.active = false;
            this.vipTerminal._submitting = false;
            Alpine.store('app').vipTerminalActive = false;
            this.viewMode = 'tools';
            this.currentGroup = 'sims';
        ...
        showVipWelcomeBack() {
            this.vipTerminal.active = true;
            Alpine.store('app').vipTerminalActive = true;
            this.vipTerminal.phase = 'welcome';
            setTimeout(() => { 
                this.vipTerminal.active = false;
                Alpine.store('app').vipTerminalActive = false;
            }, 1500);
        ...
        lockVipMode() {
            ...
            try { localStorage.removeItem('vip_granted'); } catch(e) {}
            this.viewMode = 'tools';
            this.currentGroup = 'radar';
            this.currentTab = 'dashboard';
        },

        spawnMatrixRain() {
            const container = this.$refs.matrixRain;
            if (!container) return;
            container.innerHTML = '';
            ...
            for (let i = 0; i < cols; i++) {
                ...
                let text = '';
                for (let j = 0; j < 20 + Math.floor(Math.random() * 15); j++) {
    ...
    if (this.deferredPrompt) {
        ...
        if (isIOS) {
            _toast("Tap the Share button ↑ then 'Add to Home Screen'", 'info');
        } else if (this.isMobile) {
            _toast("Tap ⋮ menu → 'Install app' or 'Add to Home Screen'", 'info');
        } else {
            _toast("Click the install icon in the address bar, or use ⋮ menu → 'Install app'", 'info');
        }
        this.dismissPwa();
    }
},
        ...
        // ------------------------------------------------------------------
        async generatePDF() {
            if (!window.jspdf) { _toast("PDF library not loaded. Please refresh.", 'error'); return; }
        ...
        async generateRoadmap() {
            if (!window.jspdf) { _toast("PDF library loading...", 'warning'); return; }
            const { jsPDF } = window.jspdf;
            const doc = new jsPDF();
...
updateTalentChart() {
    ...
    setTimeout(() => {
        ...
        const gridColor = isLight ? 'rgba(0,0,0,0.12)' : '#334155';
        const labelColor = isLight ? '#1e293b' : '#f1f5f9';
        const youColor = isLight ? '#be185d' : '#f472b6';
        const youBg = isLight ? 'rgba(190,24,93,0.15)' : 'rgba(244,114,182,0.2)';
        const stdColor = isLight ? '#64748b' : '#475569';
        const stdBg = isLight ? 'rgba(100,116,139,0.08)' : 'rgba(148,163,184,0.1)';
                ...
                datasets: [
                    {
                        ...
                        borderWidth: 2,
                        pointBackgroundColor: isLight ? youColor : '#fff',
                        pointBorderColor: youColor,
                        pointRadius: 4
                    },
                    {
...
TONE: Professional, objective, high-agency.`;
        ...
        async finishAssessment() { 
            ...
            Alpine.store('gamification').trackAssessment();
            window.scrollTo({ top: 0, behavior: 'smooth' }); 
...
async submitAndBenchmark() {
            ...
            if (!this.selectedIndustry || !this.selectedTitle) {
                _toast("Please select Organization Type and Role before benchmarking.", 'warning');
                return;
            }
            ...
            try {
                ...
                if (typeof offlineBenchmarks !== 'undefined') {
                    ...
                    _toast("Could not connect to benchmark database.", 'error');
        ...
        generateMatrixPrompt() {
            Alpine.store('gamification').trackToolUse('canvas', 'forge');
            Alpine.store('gamification').trackPromptGenerated('forge');
            const x = this.matrixCoords.x;
            const y = this.matrixCoords.y;
            const result = this.getMatrixResult();
...
TONE: Decisive, fiscally responsible, and technically sound.`;
        ...
        generateCompassPrompt() {
            Alpine.store('gamification').trackToolUse('compass', 'forge');
            Alpine.store('gamification').trackPromptGenerated('forge');
            const x = this.compassCoords.x;
            const y = this.compassCoords.y;
            const result = this.getCompassResult();
...
TONE: Visionary, urgent, but grounded in banking reality.`;
  ...
  // ---  LIGHTHOUSE ---
        ...
        generateLighthousePrompt() {
            Alpine.store('gamification').trackToolUse('lighthouse', 'forge');
            Alpine.store('gamification').trackPromptGenerated('forge');
...
// ------------------------------------------------------------------
        ...
        kpiDesigner: {
            ...
            // Navigation Actions
            next() {
                if (this.step === 1 && !this.form.project) return _toast("Please define the project first.", 'warning');
                if (this.step === 2 && !this.form.who) return _toast("Who is this for?", 'warning');
                if (this.step === 3 && !this.form.pain) return _toast("What is the pain point?", 'warning');
            ...
            generate() {
                if (!this.form.value) return _toast("Please define the business value.", 'warning');
...
TONE: High agency, executive, mathematically precise. No corporate fluff.`;
            ...
            copyPrompt() {
                if (navigator.clipboard) {
                    ...
                    _toast("Advanced Prompt copied! Paste into ChatGPT or Claude.", 'success');
                } else {
                    _toast("Clipboard access not available. Please copy manually.", 'warning');
                }
            },
        ...
        squadBuilder: {
            ...
            addRole(role) {
                if (this.roster.length >= 12) return _toast("Squad limit reached. Split the team.", 'warning');
...
TONE: Fiscally responsible, risk-averse, urgent.`;
        ...
        lighthouseROI: {
            ...
            calculate() {
                if (!this.inputs.name) return _toast("Please name the project.", 'warning');
...
TONE: Professional, financial, ruthlessly objective.`;
        ...
        conwaySim: {
            ...
            addTeam() {
                if (!this.newTeamName) return _toast("Name required.", 'warning');
                this.teams.push({
                    id: Date.now(),
...
TONE: Technical, systemic, authoritative. Quote Melvin Conway.`;
        ...
        dumbPipeCalc: {
            ...
            generateStrategyPrompt() {
                Alpine.store('gamification').trackToolUse('dumbpipe', 'radar');
...
TONE: Accountable, transparent, systems-thinking (blame the process, not the person).`;
    ...
    // ------------------------------------------------------------------
        ...
        shadowAudit: {
            ...
            addTool() {
                if (!this.form.name) return _toast("Enter tool name.", 'warning');
            ...
            generateGovernancePrompt() {
                Alpine.store('gamification').trackToolUse('shadow', 'radar');
                Alpine.store('gamification').trackPromptGenerated('radar');
...
TONE: Regulatory, precise, risk-averse.`;
        ...
        vendorCoach: {
            ...
            analyze() {
                if (!this.inputs.vendorName) return _toast("Enter vendor name.", 'warning');
...
TONE: Professional, firm, collaborative but demanding.`;
    ...
    // ------------------------------------------------------------------
        ...
        capexClassifier: {
            ...
            analyze() {
                if (!this.input.trim()) return _toast("Please paste ticket list.", 'warning');
...
TONE: Compliance-focused, protective, financially literate.`;
        ...
        legacyExplainer: {
            ...
            analyze() {
                if (!this.input.trim()) return _toast("Paste some code first.", 'warning');
...
TONE: Urgent, biological (evolutionary), prescriptive.`;
        ...
        regSimulator: {
            ...
            simulate() {
                if (!this.selectedReg) return _toast("Select a regulation.", 'warning');
...
TONE: Technical, objective, permanent record.`;
        ...
        dailyFeed: {
            ...
            requestNotify() {
                if (!("Notification" in window)) {
                    _toast("This browser does not support desktop notifications.", 'warning');
                } else {
