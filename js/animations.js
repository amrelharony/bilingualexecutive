...
const APP_ANIM = (() => {
    ...
    // ─── AUTO-HIDE ELEMENTS (unified) ──────────────────────────
    function animateAutoHide(show) {
        const toolHeader = document.querySelector('.tool-header-bar');

        if (toolHeader) {
            if (show) {
                gsap.to(toolHeader, { opacity: 1, y: 0, duration: 0.4, ease: 'power2.out', overwrite: true,
                    onStart() { toolHeader.style.pointerEvents = 'auto'; } });
            } else {
                gsap.to(toolHeader, { opacity: 0, y: -48, duration: 0.5, ease: 'power3.inOut', overwrite: true,
                    onComplete() { toolHeader.style.pointerEvents = 'none'; } });
            }
        }
    }
