...
window._isDbPausedError = function(error) {
    if (!error) return false;
    const msg = (error.message || error.msg || '').toLowerCase();
    const code = String(error.code || '');
    return msg.includes('project is paused') || msg.includes('unavailable') ||
           msg.includes('failed to fetch') || msg.includes('networkerror') ||
           code === '503' || code === 'PGRST503';
};

window._handleDbError = function(error) {
    if (window._isDbPausedError(error)) {
        const appStore = typeof Alpine !== 'undefined' && Alpine.store('app');
        if (appStore && !appStore.dbPaused) {
            appStore.dbPaused = true;
            console.warn('[Supabase] Database appears paused. Online features disabled.');
        }
        return true;
    }
    return false;
...
_applyZoom() {
    ...
    nextPage() {
        ...
        async viewPdf(chapter) {
            const finalUrl = this.getSlideUrl(chapter);
        ...
        async viewImage(chapter, type) {
            const finalUrl = this.getImageUrl(chapter, type);
...
if (window.supabase) {
    ...
    // Health-check: detect paused database (free-tier pauses after 7 days of inactivity)
    const appStore = Alpine.store('app');
    appStore.dbChecking = true;
    (async () => {
        try {
            const { error } = await this.supabase.from('guestbook_entries').select('id', { count: 'exact', head: true });
            if (error && (error.message?.toLowerCase().includes('project is paused') ||
                          error.message?.toLowerCase().includes('unavailable') ||
                          error.code === '503' || error.code === 'PGRST503')) {
                appStore.dbPaused = true;
                appStore.supabase = null;
                this.supabase = null;
                console.warn('[Supabase] Database is paused (free-tier inactivity). Online features disabled.');
            } else {
                appStore.dbPaused = false;
            }
        } catch (e) {
            if (e?.message?.includes('Failed to fetch') || e?.message?.includes('NetworkError')) {
                ...
                appStore.supabase = null;
                this.supabase = null;
                console.warn('[Supabase] Database unreachable — possibly paused.');
            }
        } finally {
            appStore.dbChecking = false;
        }
    })();

    // Keep-alive: ping DB every 4 days while users are active to prevent pause
    setInterval(async () => {
        if (!Alpine.store('app').supabase) return;
        try {
            await Alpine.store('app').supabase.from('guestbook_entries').select('id', { count: 'exact', head: true });
        } catch (_) { /* silent keep-alive */ }
    }, 4 * 24 * 60 * 60 * 1000);
